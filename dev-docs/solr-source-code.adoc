...
Download a Java 17 JDK (Java Development Kit) or later.
...
To test this, issue a "java -version" command from your shell (command prompt) and verify that the Java version is 17 or later.
The build works with any Java 17+ version (17, 21, or later); Gradle's toolchain support handles compilation with appropriate Java versions.
